...
const statusByCode: Partial<Record<ApiErrorCode, number>> = {
  VALIDATION_ERROR: 400, INVALID_OBJECT_ID: 400, INVALID_ANSWER: 400, QUESTION_NOT_IN_ATTEMPT: 400,
  NOT_FOUND: 404, ATTEMPT_NOT_FOUND: 404, USER_NOT_FOUND: 404, ATTEMPT_FORBIDDEN: 403,
  IMPORT_TOO_LARGE: 413, DUPLICATE_RESOURCE: 409, RESOURCE_IN_USE: 409, CONFLICT: 409,
  ATTEMPT_NOT_IN_PROGRESS: 409, ATTEMPT_ALREADY_SUBMITTING: 409, ATTEMPT_TIME_EXPIRED: 409,
  ATTEMPT_NO_QUESTIONS: 409, INSUFFICIENT_QUESTIONS: 409, EXAM_SET_NOT_PUBLISHED: 409,
  SUBJECT_INACTIVE: 409, TRANSACTION_REQUIRED: 503, RESULT_NOT_READY: 409, RESULT_SNAPSHOT_UNAVAILABLE: 409,
};

function statusForCode(code: ApiErrorCode): number {
  return statusByCode[code] ?? (code.startsWith("IMPORT_") ? 409 : 500);
}

export class ApiError extends Error {
  constructor(public readonly code: ApiErrorCode, message: string, public readonly details?: unknown, public readonly status = statusForCode(code)) {
    super(message);
  }
}
...
export function isTransactionUnsupportedError(error: unknown): boolean {
  ...
  const candidate = error as { message?: unknown; code?: unknown; codeName?: unknown };
  if (candidate.code === 20) return true;
  if (typeof candidate.codeName === "string" && /transaction|illegaloperation/i.test(candidate.codeName)) return true;
  return typeof candidate.message === "string" && /transaction numbers are only allowed|replica set|transactions support|transaction support/i.test(candidate.message);
}
...
export function errorResponse(error: unknown): Response {
  ...
  if (isTransactionUnsupportedError(error)) return Response.json({ success: false, error: { code: "TRANSACTION_REQUIRED", message: "MongoDB transaction yêu cầu replica set hoặc MongoDB Atlas." } }, { status: 503 });
