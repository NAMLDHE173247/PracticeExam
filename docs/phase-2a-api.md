...
`contentHash` is generated by `src/modules/questions/question-hash.ts` from normalized original question content, option labels/content/correctness, or true/false statement content/answers. Relations, status, tags, difficulty, translations, source and timestamps are excluded, so moving or editing metadata does not change duplicate identity. Development seed detects the previous seed hashes and reports them without migrating or deleting existing records.
